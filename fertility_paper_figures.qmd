...
relative_birth_rate <- df %>% group_by(state) %>% 
    ...
    ggplot(data=relative_birth_rate %>% filter(exposed_births == 0)) + 
    geom_smooth(aes(x=time, y=birthrate, group=ban, col=ban), se=FALSE) + 
    geom_smooth(aes(x=time, y=birthrate, group=ban, col=ban), se=FALSE, span=10, method="lm",
                data=relative_birth_rate %>% filter(time >= "2022-03-01", ban == "Texas")) +
    geom_smooth(aes(x=time, y=birthrate, group=ban, col=ban), se=FALSE, 
                data=relative_birth_rate %>% filter(time >= "2023-01-01", ban == "Exposed (excl. Texas)"),
                method=smooth, span=100) +
...
ggsave("~/Dropbox/abortion_results_and_data/fertility_figures/main_figures/relative_birthrate.png", width=8, height=5)

  ratio_br <- relative_birth_rate %>% pivot_wider(names_from = "ban", values_from = c("birthrate", "exposed_births"), id_cols = time) %>% 
  mutate(texas_ratio = birthrate_Texas/birthrate_Unexposed, other_ratio = `birthrate_Exposed (excl. Texas)`/birthrate_Unexposed) 
  
  method <- "loess"
  ratio_br %>% ggplot() + 
  geom_smooth(aes(x=time, y=other_ratio), se=FALSE, data = ratio_br %>% filter(time < "2023-01-01"), col="red", method=method) +
  geom_smooth(aes(x=time, y=texas_ratio), se=FALSE, data = ratio_br %>% filter(`exposed_births_Texas`==0), col="orange", method=method) +
  geom_smooth(aes(x=time, y=other_ratio), se=FALSE, data = ratio_br %>% filter(time >= "2023-01-01"), col="red", method=method, span=2) +
  geom_smooth(aes(x=time, y=texas_ratio), se=FALSE, data = ratio_br %>% filter(`exposed_births_Texas`==1), col="orange", method=method, span=4) +
  theme_bw(base_size=16) + 
  theme(legend.position = c(0.01, 0.99), legend.justification = c(0, 1),
    #legend.background = element_blank(),  # Make legend background transparent
  legend.title = element_blank()  ) +
  scale_color_manual(values=c("red", "orange", "dark gray")) + 
  ylab("Relative Birth Rate (Exposed/Unexposed)") + xlab("Year") + 
  geom_vline(xintercept=lubridate::date("2022-02-01"), color="orange", linetype="dashed") +
  geom_vline(xintercept=lubridate::date("2022-12-01"), color="red", linetype="dashed") + 
  geom_hline(yintercept=1, linetype="dashed", color="black") + scale_color_manual(values=c("red", "orange"), labels=c("Texas", "Other Exposed")) +
...
file_dir <- "~/Dropbox/abortion_results_and_data/posterior_samples/ij_interaction"
suffix <- "prior_check"
...
ggsave("~/Dropbox/abortion_results_and_data/fertility_figures/main_figures/ban_states_fit_plot.png", width=8, height=5)

make_gap_plot(quantiles_df %>% filter(type=="total"), state_name="Ban States", category="Total", target="births") + theme_bw(base_size=16)
ggsave("~/Dropbox/abortion_results_and_data/fertility_figures/main_figures/ban_states_gap_plot.png", width=8, height=5)
```
...
ggsave("~/Dropbox/abortion_results_and_data/fertility_figures/main_figures/age_interval_plot.png", width=10, height=10)
...
ggsave("~/Dropbox/abortion_results_and_data/fertility_figures/main_figures/race_interval_plot.png", width=10, height=10)
...
ggsave("~/Dropbox/abortion_results_and_data/fertility_figures/main_figures/education_interval_plot.png", width=10, height=10)
...
ggsave("~/Dropbox/abortion_results_and_data/fertility_figures/main_figures/insurance_interval_plot.png", width=10, height=10)
...
ggsave("~/Dropbox/abortion_results_and_data/fertility_figures/main_figures/marital_interval_plot.png", width=10, height=10)
...
ggsave("~/Dropbox/abortion_results_and_data/fertility_figures/main_figures/total_interval_plot.png", width=10, height=10)
...
ftab |> gtsave("~/Dropbox/abortion_results_and_data/fertility_figures/main_figures/ban_states_table.png", zoom=4)
...
for(t in types) {
  ...
  ggsave(uc_ppcs_obj$eval_plt, filename=sprintf("~/Dropbox/abortion_results_and_data/fertility_figures/supplement_figures/ppc/%s_unit_corr_plot.png", t), width=7, height=figure_height)
