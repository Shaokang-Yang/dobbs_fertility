...
all_samples %>% filter(exposure_code == 1, time=="2023-05-01") %>% 
  ...
  ggplot(aes(x=.iteration, y=te)) + geom_line(aes(color=as.factor(.chain)), alpha=0.5) + facet_wrap(~category + state, scales="free") + theme_bw()
...
# ci_df %>% group_by(state, category) %>% summarize(median_ratio = median(causal_effect_ratio)) %>% group_by(category) %>% mutate(effect_rank = rank(median_ratio)) %>% arrange(desc(effect_rank)) %>% group_by(state) %>% 
#     summarize(mean_rank = mean(effect_rank)) %>% arrange(mean_rank)

ci_df %>% group_by(state, category) %>% summarize(median_ratio = median(causal_effect_ratio)) %>% 
  group_by(category) %>% mutate(effect_rank = rank(median_ratio)) %>% 
filter(!(state %in% c("Ban States", "Ban States (excl. Texas)"))) %>% 
mutate(state = fct_reorder(state, effect_rank, .fun=mean)) %>% 
ggplot() + ggdist::geom_dots(aes(y=state, x=effect_rank), binwidth = unit(c(2, Inf), "mm")) 
...
ci_df_mean %>% filter(type != "total") %>% 
  group_by(type) %>% summarize(summary(lm(causal_effect_ratio ~ category))$adj.r.squared) -> rsquared_df
write_lines("------ Adjusted R-squareds based on Category ------- ", "figs/main_figures/summaries_for_paper.txt", append=TRUE)
write_lines(capture.output(print(rsquared_df)), "figs/main_figures/summaries_for_paper.txt", append=TRUE)
...
## Compute Effect Differences for Marital and Medicaid
...
mi_causal %>% 
    ...
    summarize(mean_rate_married = mean(Married), mean_rate_unmarried = mean(Unmarried),
              ...
              lower_diff = quantile(causal_effect_ratio_diff, 0.025), 
              upper_diff = quantile(causal_effect_ratio_diff, 0.975),
              pr_unmarried_greater = mean(Unmarried > Married)) -> pr_unmarried_greater
write_lines("------ Posterior Prob Unmarried > Married ------- ", "figs/main_figures/summaries_for_paper.txt", append=TRUE)
write_lines(capture.output(print(pr_unmarried_greater)), "figs/main_figures/summaries_for_paper.txt", append=TRUE)
...
## Portion of excess births in Texas from pre Dobbs

texas_df <- all_samples %>% filter(exposure_code == 1, state == "Texas", type == "total") %>% 
                          mutate(denom = pop, pre_dobbs = time < "2023-01-01") 
                          
texas_df %>% mutate(denom = pop * interval(start_date, end_date) / years(1)) %>%
...
write_lines("------ Fraction excess TX births from Pre Dobbs period ------- ", "figs/main_figures/summaries_for_paper.txt", append=TRUE)
...
all_samples %>% filter(ban==1, !(state %in% c("Ban States", "Ban States (excl. Texas)")), type=="total") %>% 
  ...
  ggplot() +
    ...
    xlab("Event Time")
ggsave("figs/supplement_figures/event_time_gap_plot.png", width=10, height=10)
