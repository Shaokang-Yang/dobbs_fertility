...
df %>% select(time, state, starts_with("births"), ban) %>% 
  filter(ban == 0) %>%                   
  pivot_longer(cols=starts_with('births'), names_pattern="births_(.*)", 
                    names_to="category", 
                    values_to="births") %>%
  group_by(category) %>% 
  summarize(count_missing = sum(is.na(births)), fraction_missing = round(100*count_missing / n(), 1)) %>%
