...
for(s in c("Texas", "Ban States", "Ban States (excl. Texas)")) {
  ...
  # event_time_df %>% write_csv("results/event_time_results.csv")
    ...
    ggplot() +
        ...
        scale_color_manual(values=c("red", "gray"), labels=c("Realized Treatment", "Placebo")) +
        scale_alpha_manual(values=c(1, 0.5)) +
        guides(group = "none", alpha="none", color = guide_legend(title="")) +
        theme(legend.position=c(0.2, 0.85)) +
        ylab("log(observed births) - log(expected births)")
        xlab("Event Time") + ggtitle(s) -> p
...
state_df %>%
    ...
    geom_hline(yintercept = 1.7, linetype="dashed", col="red") +
    geom_hline(yintercept = 1.0, linetype="dashed", col="blue") + ylim(c(-2, 2)) + theme_bw()
